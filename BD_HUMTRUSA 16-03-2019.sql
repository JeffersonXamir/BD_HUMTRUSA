...
BEGIN
	...
	,cv.`subtotal_con_iva`,cv.`subtotal_sin_iva`,cv.`iva_total`,cv.`descuento_total`,cv.`total` 
...
BEGIN
	...
	SELECT dv.`id`,dv.`id_cabecera_venta`,dv.`id_producto`, p.`nombre` ,dv.`precio`,dv.`cantidad`,dv.`iva`,dv.`descuento`  
